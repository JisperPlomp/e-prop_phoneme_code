...
from tools import update_plot, generate_phoneme_data
...
tf.app.flags.DEFINE_integer('n_iter', 10, 'total number of iterations')
...
#i think this is spike input??? instead of first layer of "true"" NN? not sure

phoneme_sequences = [[0, 1, 2], [0, 2, 1], [1, 2, 3]]
word_mapping = {(0, 1, 2): 0, (0, 2, 1): 1, (1, 2, 3): 2, (1,2,1):3}
#, (1,2,1):3
...
def get_data_dict(batch_size):
    ...
    spk_data, in_nums, target_data = generate_phoneme_data(batch_size, phoneme_sequences, word_mapping, num_neurons_per_phoneme=int(n_in/(n_words)),cue_spacing=t_cue_spacing)
    return {
...
with tf.name_scope('OutputComputation'):
    # Adjust the shape of the output weight matrix to accommodate three classes
    ...
    if FLAGS.eprop and FLAGS.feedback == 'random':
        # Function to generate the random feedback path with adjustments for three classes
        b_out_vals = rd.randn(n_regular + n_adaptive, n_words)  # Adjusted for 3 classes
        ...
        # Standard output calculation adjusted for 3 classes
...
for i in range(FLAGS.n_batch):
    ...
    #print(f"Test Case {i} Output Probabilities:")
    ...
    if FLAGS.do_plot:
        ...
        update_plot(plot_results_values, ax_list, n_max_neuron_per_raster=20, title=f'Test Case {i}', batch=i)
