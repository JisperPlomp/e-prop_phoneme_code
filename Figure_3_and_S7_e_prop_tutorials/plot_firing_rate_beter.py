...
print("Firing rates per word:")
for word, rates in firing_rates_per_word.items():
    print(f"Word {word}: {rates}")
...
for word, activations in neuron_activations_per_word.items():
...
def calculate_and_plot_average_firing_rate(firing_rates_per_word, title):
    ...
    iteration_count = max([len(rates) for rates in firing_rates_per_word.values()])
    ...
    for iteration in range(iteration_count):
        iter_firing_rates = []
        for word, rates in firing_rates_per_word.items():
            if len(rates) > iteration:  # Ensure that the iteration index is valid for this word
                valid_rates = rates[iteration][~np.isnan(rates[iteration])]  # Filter out NaN values
                if valid_rates.size > 0:
                    iter_firing_rates.append(np.mean(valid_rates))
        if iter_firing_rates:
            avg_firing_rate = np.mean(iter_firing_rates)
        else:
            avg_firing_rate = 0  # If there are no firing rates for this iteration, set average to 0
