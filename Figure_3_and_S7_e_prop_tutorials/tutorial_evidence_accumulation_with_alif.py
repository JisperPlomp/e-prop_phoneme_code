...
def get_data_dict(batch_size):
    ...
    spk_data, in_nums, target_data = generate_phoneme_data(batch_size, phoneme_sequences, word_mapping, num_neurons_per_phoneme=int(n_in/(n_words+1)),cue_spacing=t_cue_spacing)
    return {
...
for k_iter in range(FLAGS.n_iter):
    ...
    if np.mod(k_iter, FLAGS.print_every) == 0:
        ...
        if FLAGS.do_plot:
            ...
            plt.draw()
            plt.pause(1)
            plt.savefig(f'plot_iteration_{k_iter}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
