...
tf.app.flags.DEFINE_integer('n_iter', 300, 'total number of iterations')
...
# Experiment paraeters
t_cue_spacing = 50  # distance between two consecutive cues in ms
...
#print("Phoneme Sequences:", phoneme_sequences)
...
def generate_phoneme_data_dif_length(batch_size, phoneme_sequences, word_mapping, num_neurons_per_phoneme, cue_spacing):
    ...
    for i in range(batch_size):
        ...
        for t, phoneme_id in enumerate(seq):
            spk_data[i, t * cue_spacing:(t + 1) * cue_spacing, phoneme_id * num_neurons_per_phoneme:(phoneme_id + 1) * num_neurons_per_phoneme] = 1
...
def get_data_dict(batch_size):
    spk_data, in_nums, target_data = generate_phoneme_data_dif_length(batch_size, phoneme_sequences, word_mapping, num_neurons_per_phoneme=int(n_in / len(phoneme_to_id)), cue_spacing=15)
    return {
...
with tf.name_scope('OutputComputation'):
    ...
    temperature = 0.05 # Experiment with different values
...
with tf.name_scope('TaskLoss'):
    ...
    # Compute the weighted output logits
    weighted_output_logits = tf.reduce_sum(out[:, -t_cue_spacing:] * time_weights[None, :, :], axis=1)

    # Apply temperature scaling to logits for softmax calculation
    temperature = 0.5  # Experiment with different values
    scaled_logits = out[:, -t_cue_spacing:] / temperature  # Scale the logits

    # Apply softmax to scaled logits at each timestep
    output_probs = tf.nn.softmax(scaled_logits, axis=-1)

    # Calculate the loss using logits for cross-entropy
    ...
    # Use argmax on weighted logits for predictions
    ...
    with tf.name_scope('PlotNodes'):
        out_plot = output_probs  # Use softmax probabilities for plotting
...
test_errors = []
for i in range(FLAGS.n_batch):#kan in principe anders zijn? ipv FLAGS.n_batch
...
#start plotting plot_result_values
