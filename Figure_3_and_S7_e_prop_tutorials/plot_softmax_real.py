...
with tf.name_scope('TaskLoss'):
    time_weights = tf.constant([0.1, 0.2, 0.3, 0.4], dtype=tf.float32, shape=(t_cue_spacing, 1))

    # Compute the weighted output logits
    weighted_output_logits = tf.reduce_sum(out[:, -t_cue_spacing:] * time_weights[None, :, :], axis=1)

    # Apply softmax to logits at each timestep
    output_probs = tf.nn.softmax(out[:, -t_cue_spacing:], axis=-1)
