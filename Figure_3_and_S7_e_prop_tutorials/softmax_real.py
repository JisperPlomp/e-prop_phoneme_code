...
with tf.name_scope('TaskLoss'):
    ...
    output_probs = tf.nn.softmax(weighted_output_logits, axis=-1)
    
    # Predictions
    y_predict = tf.argmax(output_probs, axis=1)

    # Calculate accuracy
    correct_predictions = tf.equal(tf.squeeze(tiled_targets, axis=-1), y_predict)
