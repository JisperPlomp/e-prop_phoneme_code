...
for k_iter in range(FLAGS.n_iter):
    ...
    # Save neuron activations and parameters for visualization
    neuron_activations.append(results_values['av'])
    firing_rates.append(results_values['recall_errors'])
    learned_params.append(sess.run([cell.w_in_var, cell.w_rec_var, W_out]))
...
# Save neuron activation data to a file
with open("neuron_activations.txt", "w") as f:
    for activation in neuron_activations:
        f.write(f"{activation.tolist()}\n")

# Save firing rates and learned parameters
with open("firing_rates.txt", "w") as f:
    for rate in firing_rates:
        f.write(f"{rate.tolist()}\n")

with open("learned_params.txt", "w") as f:
    for params in learned_params:
        f.write(f"{[param.tolist() for param in params]}\n")
...
for i in range(FLAGS.n_batch):
    ...
    if True:
        ...
        correct_decision = plot_results_values['target_nums'][i, -1]
